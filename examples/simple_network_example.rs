...
//! **Run this example**:
//! ```bash
//! cargo run --example simple_network_example
//! ```
...
use zznet_session::RoomMessageTrait;
...
impl RoomMessageTrait for AppMessage {
        ...
        Self: Sized,
    {
        ...
        if s.starts_with("Heartbeat") {
            Ok(AppMessage::Heartbeat { sequence: 0 })
        } else if s.starts_with("DataRequest") {
            Ok(AppMessage::DataRequest {
                query: "parsed".to_string(),
            })
        } else if s.starts_with("DataResponse") {
            Ok(AppMessage::DataResponse { data: vec![] })
        } else {
            Ok(AppMessage::StatusUpdate {
