...
pub fn compress_chunked_v1(records: &[RawDataRecord]) -> Result<Vec<u8>> {
    ...
    let mut chunks: std::collections::BTreeMap<u64, Vec<RawDataRecord>> = std::collections::BTreeMap::new();
    for record in records {
        chunks.entry(record.sent_nanos / 60_000_000_000).or_default().push(*record);
    }
    ...
    for chunk_records in chunks.values() {
        ...
        index_entries.push(IndexEntry {
            chunk_offset_bytes: (HEADER_SIZE + payload_buffer.len() + asterisk_delimiter.len()) as u64,
        });

        payload_buffer.extend_from_slice(asterisk_delimiter);
        payload_buffer.extend_from_slice(&chunk_buffer);
        payload_buffer.extend_from_slice(&chunk_data_crc32.to_be_bytes());
        payload_buffer.extend_from_slice(asterisk_delimiter);
    }
