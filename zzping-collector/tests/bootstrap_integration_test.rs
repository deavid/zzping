...
"#
    ...
    tokio::time::timeout(Duration::from_millis(1500), async {
        loop {
            if check_logs()
                .iter()
                .any(|s| s.contains("Attempting to connect"))
            {
                break;
            }
    ...
    .await
    .expect("Collector did not connect initially.");
    ...
    tokio::time::timeout(Duration::from_millis(1500), async {
        loop {
            ...
            let reconnect_attempt = logs
                .iter()
                .filter(|s| s.contains("Attempting to connect"))
                .count()
                >= 2;
    ...
    .await
    .expect("Collector did not attempt to reconnect after server shutdown.");
