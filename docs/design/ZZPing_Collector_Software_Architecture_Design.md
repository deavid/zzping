...
1. **Bootstrap (main.rs):**
   ...
   * main.rs loads collector.ron to get its hostname and the database address.
...
5. **New Session (CollectorService & SessionHandler):**
   ...
   * The new SessionHandler establishes its gRPC loops. The database recognizes the collector hostname and again promotes it to PRIMARY.
...
2. **New Instance Starts (main.rs):** A new collector instance (C2) is launched for an upgrade.
   ...
   * C2 starts its CollectorService and connects to the database, identifying with the same hostname. It starts in the STANDBY role.
