...
impl<T: ApplicationRole> MemDBBuilder<T> {
    ...
    pub fn start(self) -> Result<Addr<MemDBActor<T>>, Error> {
        // Validate configuration
        self.role.validate()?;

        // Start actor
        let actor = MemDBActor {
            role: self.role,
...
**`actor.rs`** - Implementation:
```rust
pub struct MemDBActor<T: ApplicationRole> {
    role: MemDBRole,
    session_manager: Option<Rc<SessionManager<MemDBMessage, PermissionWrapper<T>>>>,
    // Private state based on role
    buffer: VecDeque<PingResult>,      // Used by Collector role
    storage: HashMap<IpAddr, Vec<...>>, // Used by Database role
}
...
impl<T: ApplicationRole> Actor for MemDBActor<T> {
    ...
    fn started(&mut self, ctx: &mut Self::Context) {
        if let Some(ref sm) = self.session_manager {
            ...
            sm.subscribe_to_events(ctx.address().recipient());
        }
    }
}
...
impl<T: ApplicationRole> Handler<MemDBMessage> for MemDBActor<T> {
    ...
    fn handle(&mut self, msg: MemDBMessage, _ctx: &mut Context<Self>) {
        match (&self.role, msg) {
            (MemDBRole::Collector { .. }, MemDBMessage::QueryResponse { results }) => {
                // Collector receiving query response
                self.process_query_response(results);
            }
            (MemDBRole::Database { .. }, MemDBMessage::SubmitBatch { results }) => {
                // Database receiving batch submission
                self.store_batch(results);
            }
            (MemDBRole::Database { .. }, MemDBMessage::Query { target, time_range }) => {
                // Database handling query
                let results = self.execute_query(target, time_range);
            ...
            _ => {
                log::warn!("Unexpected message for role: {:?}", msg);
            }
        }
    }
}
...
impl<T: ApplicationRole> Handler<SessionEvent> for MemDBActor<T> {
    type Result = ();
    ...
    fn handle(&mut self, event: SessionEvent, _ctx: &mut Context<Self>) {
        match event {
            SessionEvent::Active { peer_id, .. } => {
                ...
                // Component can react to new connection if needed
            }
            SessionEvent::Inactive { peer_id } => {
                log::warn!("Disconnected from peer: {}", peer_id);
                // Component can clean up per-connection state if needed
            }
        }
    }
}
...
```rust
// src/components/my-component/src/network_messages.rs
#[derive(Debug, Clone, Serialize, Deserialize, Message)]
#[rtype(result = "()")]
...
**Step 2: Define roles**
```rust
// src/components/my-component/src/role.rs
#[derive(Debug, Clone)]
pub enum MyComponentRole {
    Client,
...
```rust
// src/components/my-component/src/actor.rs
pub struct MyComponentActor<T: ApplicationRole> {
    role: MyComponentRole,
    session_manager: Option<Rc<SessionManager<MyComponentMessage, PermissionWrapper<T>>>>,
}

impl<T: ApplicationRole> Handler<MyComponentMessage> for MyComponentActor<T> {
    type Result = ();

    fn handle(&mut self, msg: MyComponentMessage, _ctx: &mut Context<Self>) {
        match (&self.role, msg) {
            (MyComponentRole::Client, MyComponentMessage::Response { result }) => {
                // Handle response
            }
            (MyComponentRole::Server { .. }, MyComponentMessage::Request { data }) => {
                // Handle request, send response
                if let Some(ref sm) = self.session_manager {
                    sm.send_to_room(
                        peer_id,
                        RoomId::from("my-component"),
                        MyComponentMessage::Response { result: 42 }
                    );
                }
            }
            _ => {}
...
```rust
// src/components/my-component/src/builder.rs
impl<T: ApplicationRole> MyComponentBuilder<T> {
    pub fn new(role: MyComponentRole) -> Self { /* ... */ }
    pub fn with_session_manager(self, sm: ...) -> Self { /* ... */ }
    pub fn start(self) -> Result<Addr<MyComponentActor<T>>, Error> { /* ... */ }
}
```
...
```rust
// In main.rs
let session_manager = SessionManager::new(vec!["my-component"]).start();

let my_component = MyComponentBuilder::new(MyComponentRole::Client)
    .with_session_manager(Rc::new(session_manager))
    .start()?;
```
...
async fn test_component_logic() {
    // Create component without SessionManager
    let component = MyComponentBuilder::new(MyComponentRole::Server { .. })
        .start()
        .unwrap();

    // Send messages directly to actor
    let result = component.send(SomeLocalMessage { .. }).await.unwrap();
...
async fn test_component_communication() {
    // Create two SessionManagers with mock connector
    let sm_client = SessionManager::new(vec!["my-component"]).start();
    let sm_server = SessionManager::new(vec!["my-component"]).start();

    // Connect them via mock (no network I/O!)
    let mock = MockConnector::connect(&sm_client, &sm_server);

    // Create components
    let client = MyComponentBuilder::new(MyComponentRole::Client)
        .with_session_manager(Rc::new(sm_client))
        .start()?;

    let server = MyComponentBuilder::new(MyComponentRole::Server { .. })
        .with_session_manager(Rc::new(sm_server))
        .start()?;

    // Simulate connection
    sm_client.peer_connected("server");
    sm_server.peer_connected("client");

    // Send message and verify
    client.send(SendRequest { data: "test".into() }).await?;

    // Verify server received and responded
    // (via test message inspection or mock connector)
}
```
