...
mod tests {
    ...
    fn test_database_config() {
        let config = IntentConfigConfig::for_database(PathBuf::from("/etc/intent.ron"));
        assert!(config.persist_config);
        assert!(config.accept_config_changes);
        assert!(config.broadcast_config_updates);
        assert!(config.validate().is_ok());
    }

    #[test]
    fn test_collector_config() {
        let config = IntentConfigConfig::for_collector();
        assert!(!config.persist_config);
        assert!(!config.accept_config_changes);
        assert!(!config.broadcast_config_updates);
        assert!(config.validate().is_ok());
    ...
    fn test_default_is_collector() {
        let config = IntentConfigConfig::default();
        assert_eq!(config, IntentConfigConfig::for_collector());
    }
}
