...
mod tests {
    ...
    fn test_default_permissions() {
        let perms = MemDBPermissions::default();
        assert!(!perms.can_submit_batch);
        assert!(!perms.can_query);
        assert!(!perms.can_receive_batches);
    }

    #[test]
    fn test_collector_permissions() {
        let perms = MemDBPermissions::collector();
        assert!(perms.can_submit_batch);
        assert!(!perms.can_query);
        assert!(!perms.can_receive_batches);
    }
    ...
    #[test]
    fn test_database_permissions() {
        let perms = MemDBPermissions::database();
        assert!(!perms.can_submit_batch);
        assert!(perms.can_query);
        assert!(perms.can_receive_batches);
    }
    ...
    fn test_admin_permissions() {
        let perms = MemDBPermissions::admin();
        assert!(perms.can_submit_batch);
        assert!(perms.can_query);
        assert!(perms.can_receive_batches);
    }
