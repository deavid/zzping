...
impl PeerChannelsBuilder {
    ...
    pub(crate) fn build_and_spawn_loop(self, inbound_rx: mpsc::Receiver<(RoomId, Vec<u8>)>) {
        let rooms_map = self.rooms;

        let rooms = rooms_map;
        let peer_id = self.peer_id.clone();
        tokio::spawn(inbound_task_loop(rooms, peer_id, inbound_rx));
    }
}
...
async fn inbound_task_loop(
    ...
    while let Some((room_id, bytes)) = inbound_rx.recv().await {
        route_inbound_message(&rooms, &peer_id, room_id, bytes).await;
    }
