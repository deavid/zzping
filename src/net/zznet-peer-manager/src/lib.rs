...
/// SharedPeerRegistry - adapter that allows sharing a PeerManager behind a
/// Mutex and exposing the PeerRegistry trait as a dyn object.
///
/// This is useful for application composition where components expect
/// Arc<dyn PeerRegistry> but the control-plane PeerManager is stored
/// behind Arc<Mutex<PeerManager>> and also wrapped by PeerManagerActor.
pub struct SharedPeerRegistry(pub Arc<Mutex<PeerManager>>);

impl zznet_api::traits::PeerRegistry for SharedPeerRegistry {
    fn get_peer_role(&self, peer_id: &PeerId) -> Option<Role> {
        let m = self.0.lock().unwrap();
        m.get_peer_role(peer_id).cloned()
    }

    fn get_peer_identity(&self, peer_id: &PeerId) -> Option<PeerIdentity> {
        let m = self.0.lock().unwrap();
        m.get_peer_identity(peer_id).cloned()
    }

    fn peers_with_role(&self, role: &Role) -> Vec<PeerId> {
        let m = self.0.lock().unwrap();
        m.peers_with_role(role)
    }
...
// Canonical types are re-exported from zznet-api to keep the facade stable.
