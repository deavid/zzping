...
async fn test_harness_run_services_connect_and_message() {
    ...
    local
        .run_until(async move {
            ...
            let (handle_a, service_a, comp_a_addr, stop_a) =
                spawn_demo_service_with_builder(cfg_a.clone()).await;
            let (handle_b, service_b, _comp_b_addr, stop_b) =
                spawn_demo_service_with_builder(cfg_b.clone()).await;
